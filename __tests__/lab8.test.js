describe('Basic user flow for SPA ', () => {
  ...
  it('Test4: On first Entry page - checking page header title', async () => {
    ...
    const header = await page.$eval('header > h1', (entry) => { return entry.innerHTML; });
  ...
  it('Test5: On first Entry page - checking <entry-page> contents', async () => {
    /*
      ...
      */
     const entry = await page.$eval('body > entry-page', e => e.entry);
     const entryData = {
       title: 'You like jazz?',
       date: '4/25/2021', 
       content: "According to all known laws of aviation, there is no way a bee should be able to fly. Its wings are too small to get its fat little body off the ground. The bee, of course, flies anyway because bees don't care what humans think is impossible.",
  ...
  it('Test8: On Settings page - checking page header title', async () => {
    ...
    const hdr = await page.$eval('header > h1', (header) => {
      return header.innerHTML;
    });
    expect(hdr).toBe('Settings');
  });

  it('Test9: On Settings page - checking <body> element classes', async () => {
    // implement test9: Clicking on the settings icon should update the class attribute of <body> to ‘settings’
    const settings = await page.$eval('body', (e) => {
      return e.classList;
    });
  ...
  it('Test13: On the home page the <body> element should not have any class attribute', async() => {
    const noClassAtt = await page.$eval('body', e => e.className);
    expect(noClassAtt).toBe("");
  });
  ...
  it('Test15: Verify the title is current when clicking on the second entry', async() => {
    //await page.click('journal-entry:nth-child(2)');
    const header = await page.$eval('body > header > h1', e => e.textContent);
  ...
  it('Test16: Verify the entry page contents is correct when clicking on the second entry', async() => {
    //await page.click('journal-entry:nth-child(2)');
    const content = await page.$eval('body > entry-page', e => e.entry);
    const contentReal = { 
